((root, factory) => {
  ...
  function install() {
    // k3s divergence from the Home Assistant add-on, and the ONLY one:
    // behind Nginx Proxy Manager the app is served at / on its own vhost, so
    // there is no ingress prefix to gate on. Install unconditionally, guarded
    // only by the idempotence flag.
